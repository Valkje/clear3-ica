...
The data of the multilevel imputations have been scaled (well, only de-meaned, really) to ease mixed imputation model estimation, but for the final, to-be-pooled models I would like at least mean_accuracy to be scaled back to its original scale.
...
```{r}
comp <- mice::complete(imp_multi, 1) %>%
...
m1 <- glmer(mean_accuracy ~ . - subject + (1 | subject), 
            data = comp,
            family = "binomial")
...
m1_bobyqa <- glmer(mean_accuracy ~ . - subject + (1 | subject), 
                   data = comp,
                   family = "binomial",
                   control = glmerControl("bobyqa",
                                          optCtrl = list(
                                            maxfun = 100 * ncol(comp)^2
                                          )))
...
```{r}
imp_models <- mice::complete(imp_multi, "long") %>%
  mutate(mean_accuracy = mean_accuracy + mean_acc) %>%
  group_by(.imp) %>%
  nest() %>%
  rowwise() %>%
  mutate(model = list(glmer(mean_accuracy ~ . - subject - .id + (1 | subject), 
                            data = data,
                            family = "binomial")))

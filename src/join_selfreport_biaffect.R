join_selfreport_biaffect <- function(
    selfreport_path, inclusion_path,
    weekly = FALSE, only_twohanded = FALSE, include_tdia = FALSE
) {
  # Additions to the file name. We will build up the vector as the program
  # progresses.
  tags <- c()

  ### Clean self-report data

  dat_rep_full <- read_sav(selfreport_path)
  dat_rep_full <- zap_labels(dat_rep_full)

  # Save it to a format that is more easily accessible
  fname <- str_match(basename(selfreport_path), "(.*)\\..*")[,2]
  write.csv(dat_rep_full,
            file.path(dirname(selfreport_path), paste0(fname, ".csv")))
  ...
  inclusion <- read_excel(inclusion_path)$Included
  ...
  if (weekly) {
    ...
    # First bind TDIAs to BiAffect data, then bind that to self-report data. If
    # we would bind (weekly) TDIAs to the self-report data first, we'd introduce
    # `NA` dates into the self-report data.
    if (include_tdia) {
      dat_bound <- dat_day %>%
    ...
    if (include_tdia) {
      # Join TDIA with BiAffect features based on date and subject
      dat_bound <- dat_day %>%
        full_join(sub_tdia, by = c("subject", "date"))
    } else {
      dat_bound <- dat_day
    }

    # Join TDIA+BiAffect with self-report measures
    dat_reg <- dat_bound %>%
      full_join(dat_rep_reg, by = c("subject" = "id", "date" = "daterated"))
  }
  ...
  if (tags != "")
    file_name <- str_glue("dat_reg_{tags}.rds")
  else
    file_name <- "dat_reg.rds"

  message(paste("Saving", file_name))

  saveRDS(dat_reg, file = file.path(dat_dir, file_name))

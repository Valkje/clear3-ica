...
```{r}
n_days <- nrow(dat_day)
cors <- matrix(nrow = n_days, ncol = n_days)

pb <- txtProgressBar(max = n_days*n_days, style = 3)

for (i in 1:n_days) {
  for (j in i:n_days) {
    setTxtProgressBar(pb, (i-1)*n_days+j)
    
    cors[i, j] <- cor(as.vector(dat_day[i,]$hist[[1]]), as.vector(dat_day[j,]$hist[[1]]))
  }
}
...
```{r}
lag_cors <- melt(cors, varnames = c("i", "j")) %>%
  mutate(lag = abs(j - i)) %>%
  group_by(lag) %>%
  summarize(
...
for (i in 1:length(subjects)) {
  ...
  g <- ggplot(lag_cors, aes(lag, value)) +
    geom_line() +
    geom_point() +
    ylim(0, 1)
...
# ggsave("~/Documents/Autocorrelation/images/bootstrap_tdia_per_subject.png", 
#        dpi = 100)
```
...
```{r}
load(file.path(dat_dir, "tdias_permuted.rda"))
```
...
ggsave("~/Documents/Autocorrelation/images/permuted_tdia_per_subject.png",
       dpi = 100)
```
...
ggplot(iter_all_means, aes(iter_mean, fill = permuted)) +
  geom_histogram(color="#e9ecef", bins = 100, alpha = 0.8) +
  xlab("Mean TDIA") +
  ylab("Count") +

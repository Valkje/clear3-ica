...
hists
# kl_div <- select(hists, year, month, week, KL_divergence)
```
...
ggpairs(dat_ses %>% 
          ...
          select(!c(sessionNumber, hour, sessionTimestampLocal, date)),
        alpha = 0.1)
```
...
```{r}
dat_rep <- read.csv("../CLEAR3_Biaffect_Leow/Archived Versions/2022-02-15 - Updated Daily Dataset for Elena and Alex/20220215_clear3daily.csv")
head(dat_rep)
```
...
```{r}
ggplot(dat_rep, aes(x = wishsleep, y = irritability_mean)) +
  geom_bin_2d()
```

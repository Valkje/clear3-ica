...
plot_validation <- function(model_df) {
  ...
  lapply(model_df, function(m) print(plot(m[[1]], main = "Residuals vs. fitted values")))
  
  # Normality of residuals.
  
  lapply(model_df, function(m) {
    m <- m[[1]]
    qqnorm(resid(m), main = "Residuals")
    qqline(resid(m))
  })
  ...
  lapply(model_df, function(m) {
    ...
    qqnorm(refs, main = "Subject-level random effects")
    qqline(refs)
  })
  ...
  lapply(model_df, function(m) {
    ...
    qqnorm(refs, main = "Week-within-subjects-level random effects")
    qqline(refs)
  })
}
```

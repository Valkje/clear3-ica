...
# Plot residuals and random effects for model validation.
plot_validation <- function(model_df) {
  col_names <- colnames(model_df)
  
  # Check for heteroskedasticity of residuals.
  
  map2(model_df, col_names, function(m, n) print(plot(m[[1]], main = paste0(n, ": Residuals vs. fitted values"))))
  
  # Normality of residuals.
  
  map2(model_df, col_names, function(m, n) {
    m <- m[[1]]
    qqnorm(resid(m), main = paste0(n, ": Residuals"))
    qqline(resid(m))
  })
  ...
  map2(model_df, col_names, function(m, n) {
    ...
    qqnorm(refs, main = paste0(n, ": Subject-level random effects"))
    qqline(refs)
  })
  ...
  map2(model_df, col_names, function(m, n) {
    ...
    qqnorm(refs, main = paste0(n, ": Week-within-subjects-level random effects"))
    qqline(refs)
  })
}
```
...
```{r}
dat_reg_frag %>% group_by(variable) %>% summarize(n = n_distinct(value))
```
...
subs <- ls$subs
dates <- ls$dates
dats_c_no_tdia <- ls$dats_c
```

```{r}
no_biaffect <- setdiff(dat_sr$subject, dat_bi$subject)
saveRDS(no_biaffect, file.path(dat_dir, "participants_without_biaffect.rds"))
```


```{r}
write_rds(as.integer(unique(dat_sr$subject)), 
          file.path(dat_dir, "participants_in_ICA.rds"))

write_rds(as.integer(unique(dats_c_no_tdia[[1]]$subject)), 
          file.path(dat_dir, "participants_in_LMER.rds"))
```

Histogram of suicidality items:

```{r fig.height=8}
dat_tmp <- dat_sr %>%
  select(ASIQ9_wishdead:mastery) %>%
...
ggsave(file.path(man_img_dir, "self-report_hists.png"))
```


```{r}
dat_asiq <- dat_sr %>%
  select(starts_with("ASIQ")) %>%
  pivot_longer(everything(), names_to = "variable")
...
ggsave(file.path(man_img_dir, "asiq_hists.png"))
```
...
```{r}
# Data fed to mixed-effects models
dats_c_no_tdia[[1]] %>%
  group_by(subject) %>%
  summarize(n = n()) %>%
  summarize(avg = mean(n), sd = sd(n))
...
```{r}
dat_c1_no_tdia <- dats_c_no_tdia[[1]]

m5.no_tdia.X1.nlme <- lme(X1 ~ medianIKD +
                           percent95IKD +
                           madIKD +
                           autocorrectRate +
                           backspaceRate +
...
m5.no_tdia.all.nlme <- dat_c1_no_tdia %>%
  summarize(across(starts_with("X", ignore.case = FALSE), 
                   ~ list(update(m5.no_tdia.X1.nlme, 
                                 reformulate(".", cur_column())))))
```

```{r}
plot_validation(m5.no_tdia.all.nlme)
```

```{r paged.print=FALSE}
lapply(m5.no_tdia.all.nlme, function(m) summary(m[[1]]))
```

```{r}
lapply(m5.no_tdia.all.nlme, function(m) intervals(m[[1]]))
```
...
```{r}
lapply(m5.no_tdia.all.nlme, function (m) {
  m <- m[[1]]
  m.b <- update(m, random = ~ 1 | subject)
  anova(m, m.b)
})
```

```{r paged.print=FALSE}
summary(m5.no_tdia.all.nlme[[2]][[1]])
```
...
ls <- prep_reg_dat(
  dat_path = file.path(dat_dir, "dat_reg_fragmented_no-tdia.rds"),
  ica_path = file.path(dat_dir, "dat_reg_frag_icas_norm.rda")
)

dat_reg_no_tdia_norm <- ls$dat_reg
dat_bi <- ls$dat_bi
dat_sr <- ls$dat_sr
icas <- ls$icas
subs <- ls$subs
dates <- ls$dates
dats_c_no_tdia_norm <- ls$dats_c
```

```{r}
dat_c1_no_tdia_norm <- dats_c_no_tdia_norm[[1]]

m5.no_tdia.norm.X1.nlme <- lme(X1 ~ medianIKD +
                                 percent95IKD +
                                 madIKD +
                                 autocorrectRate +
                                 backspaceRate +
                                 ...
                                 upright,
                               random = ~ 1 | subject / week,
                               data = dat_c1_no_tdia_norm)
```

```{r paged.print=FALSE}
m5.no_tdia.norm.all.nlme <- dat_c1_no_tdia_norm %>%
  summarize(across(starts_with("X", ignore.case = FALSE), 
                   ~ list(update(m5.no_tdia.norm.X1.nlme, 
                                 reformulate(".", cur_column())))))

lapply(m5.no_tdia.norm.all.nlme, function(m) summary(m[[1]]))
```

```{r}
estimates_to_csv(file.path(dat_dir, "m5_nlme_p_frag_no-tdia_norm.csv"),
                 m5.no_tdia.norm.all.nlme,
                 include_corrected = TRUE)
```
...
```{r}
dat_c2_no_tdia <- dats_c_no_tdia[[2]]

m10.no_tdia.X1.nlme <- lme(X1 ~ medianIKD +
                             percent95IKD +
                             madIKD +
                             autocorrectRate +
                             backspaceRate +
...
m10.no_tdia.all.nlme <- dat_c2_no_tdia %>%
  summarize(across(starts_with("X", ignore.case = FALSE), 
                   ~ list(update(m10.no_tdia.X1.nlme, 
                                 reformulate(".", cur_column())))))
```

```{r}
plot_validation(m10.no_tdia.all.nlme)
```

## 20 components

```{r}
dat_c3_no_tdia <- dats_c_no_tdia[[3]]

m20.no_tdia.X1.nlme <- lme(X1 ~ medianIKD +
                             percent95IKD +
                             madIKD +
                             autocorrectRate +
                             backspaceRate +
...
m20.no_tdia.all.nlme <- dat_c3_no_tdia %>%
  summarize(across(starts_with("X", ignore.case = FALSE), 
                   ~ list(update(m20.no_tdia.X1.nlme, 
                                 reformulate(".", cur_column())))))
```

```{r}
plot_validation(m20.no_tdia.all.nlme)
```
...
```{r}
dat_all_no_tdia <- dat_reg_no_tdia %>%
  mutate(
    variable = if_else(modality == "self_report", paste0("X.", variable), variable),
    value = case_when(
      modality == "self_report" & min(value) == 0 ~ log(value + 1), # To prevent log(0) = NA values
      modality == "self_report" ~ log(value),
      TRUE ~ value
    ),
    week = week(date)
  ) %>%
...
```{r}
mAll.no_tdia.X1.nlme <- lme(X.ASIQ9_wishdead ~ medianIKD +
                             percent95IKD +
                             madIKD +
                             autocorrectRate +
                             backspaceRate +
                             ...
                             upright,
                           random = ~ 1 | subject / week,
                           data = dat_all_no_tdia)

mAll.no_tdia.all.nlme <- dat_all_no_tdia %>%
  summarize(across(starts_with("X", ignore.case = FALSE), 
                   ~ list(update(mAll.no_tdia.X1.nlme, 
                                 reformulate(".", cur_column())))))
```

```{r paged.print=FALSE}
estimates_to_csv(file.path(dat_dir, "mAll_nlme_p_frag_no-tdia.csv"),
                 mAll.no_tdia.all.nlme,
                 include_corrected = TRUE)
```

```{r}
plot_validation(mAll.no_tdia.all.nlme)
```
